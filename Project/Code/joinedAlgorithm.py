...
def loadImages(inputDir: Path):
    ...
    for file in inputDir.iterdir(): # Loops through the entire directory
        if file.suffix.lower() in [".png", ".jpg", ".jpeg", ".bmp", ".tiff"]: # We'll only check for the most common imagetypes
...
def convertToPngIfNeeded(imgPath: Path, outputDir: Path):
    ...
    try:
        with Image.open(imgPath) as im: # Uses image.open to use metadata to convert to PNG
            exifData = im.info.get("exif") # Gets exif data
            iccProfile = im.info.get("icc_profile") # Gets icc profile
